...
| `gfx/widgets` | Shared Shadow-UI-style components built on `gfx`/`gfx/text`: `Theme` (named color palette), `SoftButton`/`SoftButtonState`/`DrawBotStrip` (semantic button state instead of matching on label text), `ListRow`/`ListView`/`RenderList` (scrollable list + breadcrumb + scrollbar), `KVRow`/`DrawKVRows` (label:value rows), plus ahead-of-need primitives (`DrawBorder`/`DrawMeter`/`DrawArc`, a `Knob` type). Operates only on plain `image.NRGBA` — no shm, no hack-specific state — so it's usable by push-manager's Shadow UI, keyboard-visualizer's renderer, or any future screen-drawing hack. All 4 of push-manager's Shadow UI panels (Files/Stats/Midi/Browser) are migrated onto this; see `../discovery/shadow-ui-component-framework.md`. |
