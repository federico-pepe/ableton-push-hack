...
# Read a flat top-level scalar field from hack.json (string/number/bool).
# hack.json files in this repo are simple, 2-space-indented, flat JSON —
# no external jq/python dependency needed. Does not handle nested objects
# with a key of the same name (none currently exist).
hack_json_field() {
    local json_file="$1"
    local field="$2"
    local line value
    line=$(grep -m1 -E "^[[:space:]]*\"${field}\"[[:space:]]*:" "${json_file}") || return 0
    value="${line#*:}"
    value="${value%,}"
